...
namespace NesCPU
{
	...
	std::uint8_t  S  = 0xFF; //Stack pointer register (SP)
	std::uint8_t  A  = 0x00;
	std::uint8_t  X  = 0x00;
	std::uint8_t  Y  = 0x00;
	...
	std::uint16_t* NmiHandlerAddr   = (uint16_t*)Memory + 0xFFFA; //$FFFA–$FFFB
	std::uint16_t* ResetHandlerAddr = (uint16_t*)Memory + 0xFFFC; //$FFFC–$FFFD
	std::uint16_t* BreakHandlerAddr = (uint16_t*)Memory + 0xFFFE; //$FFFE–$FFFF
	...
	//Addressing modes
	void abs();  //Absolute  --Change name to prevent conflict?--
	...
	void TSX(); //Transfer S to X
	void TXS(); //Transfer X to S
