...
namespace nesremu
{
    ...
    void NesEmu::start(std::istream& romStream)
    {
        ...
        m_bus = new NesBus();
        //m_bus->map(m_rom, ROM_ADDR, ROM_SIZE, ROM_ADDRESS_FORMATTER, MAP_RW);
        m_bus->map(m_ram, CPU_RAM_ADDR, CPU_RAM_SIZE, CPU_RAM_ADDRESS_FORMATTER, MAP_RW);
        m_bus->map(m_controller1, CONTROLLER1_ADDR, 1, NONE_ADDRESS_FORMATTER, MAP_RW);
        m_bus->map(m_controller2, CONTROLLER2_ADDR, 1, NONE_ADDRESS_FORMATTER, MAP_READ); //Writes to 0x4017 go to the APU.
        
        //m_apu = new NesApu(m_bus);
        //m_cpu = new NesCpu(m_bus);
        //m_ppu = new NesPpu(m_bus);
    ...
    void NesEmu::stop()
    {
        ...
        //Debugging.
        if (m_logEnabled) {
            *m_logStream << "Emu: Exiting..." << std::endl;
        }
    ...
    void NesEmu::enableLog(uint16_t flags)
    {
        ...
        if (m_state != NesEmuState::STOPPED) {
            ...
            if (flags & LogFlags::LOG_BUS) m_bus->enableLog();
    ...
    void NesEmu::disableLog(uint16_t flags)
    {
        ...
        if (m_state != NesEmuState::STOPPED) {
            ...
            if (flags & LogFlags::LOG_BUS) m_bus->disableLog();
